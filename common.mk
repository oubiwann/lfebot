...
check-unit-with-deps: get-deps compile check-unit-only
...
check-all-with-deps: compile check-unit-only check-integration-only \
	check-system-only
check-all: get-deps compile-no-deps check-unit-only check-integration-only \
	check-system-only
...
upload: $(EXPM) get-version
	@echo "Preparing to upload lfebot ..."
	@echo
	@echo "Package file:"
	@echo
	@cat package.exs
